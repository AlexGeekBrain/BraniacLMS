...
class TestNewsPage(TestCase):
    ...
    def setUp(self):
        ...
        self.user_admin = authapp_models.User.objects.get(username="admin")
...
class TestCoursesWithMock(TestCase):
    fixtures = (
        "authapp/fixtures/user_admin.json",
        "mainapp/fixtures/courses.json",
        "mainapp/fixtures/lessons.json",
        "mainapp/fixtures/teachers.json",
    )

    def test_page_open_detail(self):
        course_obj = mainapp_models.Course.objects.get(pk=1)
        path = reverse('mainapp:courses_detail', args=[course_obj.pk])
        with open('mainapp/fixtures/feedback_list_1.bin', 'rb') as inpf, mock.patch(
